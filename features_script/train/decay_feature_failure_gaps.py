# -*- coding: utf-8 -*-
"""
Created on Fri Jun 10 13:52:33 2016
...
def preprocess_data(temp):
    ...
    temp.ix[:,'date'] = dn+1
    temp = temp[['date','district_id','time_of_a_day','successful_order','success']].copy()
    temp.ix[:,'time_slot'] = 1
    temp.ix[:,'time_slot'] = temp.apply(lambda row:make_minute_slot(row['time_of_a_day']),axis = 1)
    temp = temp[['date','district_id','time_slot','successful_order','success']].copy()
    return temp
...
gaps_days_list = []
#for n in range(21):
for dn in range(9,12):
    temp = pd.read_csv('order_data_2016-01-'+dates[dn]+'.csv')
    neworder = pd.read_csv('neworder_data_2016-01-'+dates[dn]+'.csv')
    temp['success'] = pd.Series(neworder['success'],index = temp.index)
    temp = temp[['date','start_district_hash','time_of_a_day','driver_id','success']]
    print '*'*50;print '--------start prepare data for date '+str(dates[dn])+ '  -------'; print '*'*50;
    temp = preprocess_data(temp)
    temp = temp[temp['success'] == 0].copy()
    gaps_days_list.append(get_day_gaps(temp,dn))
    
gaps_decay = pd.concat(gaps_days_list,axis = 0)
gaps_decay.to_csv('gaps_decay_failure_9_11.csv',index = False)
