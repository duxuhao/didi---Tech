...
made = 1
coming = 2
datelist = np.arange(23,32,2)

for i in datelist:
    print 'date ' + str(i)
    T = []
    filename = 'test_set_2/order_data/order_data_2016-01-' +str(i)  + '_test'
    f = open(filename,'r')
...
T = []
f = open('test_set_2/cluster_map/cluster_map','r')
while 1:
    a = f.readline()
    if a == '':
        break
...
for i in datelist:
    T = []
    filename = 'test_set_2/traffic_data/traffic_data_2016-01-' +str(i)   + '_test'
    f = open(filename,'r')
    while 1:
        a = f.readline()
        if a == '':
...
for i in datelist:
    T = []
    filename = 'test_set_2/weather_data/weather_data_2016-01-' +str(i)  + '_test'
    f = open(filename,'r')
    while 1:
        a = f.readline()
        if a == '':
...
weather.timeslice = weather.timeslice * 5
weather2 = weather.copy()
weather2.timeslice += -1
weather = pd.concat([weather,weather2])
weather2.timeslice += -2
weather = pd.concat([weather,weather2])
weather2.timeslice += -3
weather = pd.concat([weather,weather2])
weather2.timeslice += -4
weather = pd.concat([weather,weather2])
print weather.columns
test = pd.merge(Train, weather, on=['timeslice','date'], left_index = True, how = 'left')
test.to_csv('Testdata_2_10.csv',encoding = 'utf-8', index = None)
